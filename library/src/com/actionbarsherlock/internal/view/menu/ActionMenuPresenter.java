...
public class ActionMenuPresenter extends BaseMenuPresenter
	...
	private class ActionButtonSubmenu extends MenuPopupHelper {
		...
		public ActionButtonSubmenu(final Context context, final SubMenuBuilder subMenu) {
	...
	private class OverflowMenuButton extends ImageButton implements ActionMenuChildView, View_HasStateListenerSupport {
		private final boolean								mForceDividerToBeVisible;
		...
		public OverflowMenuButton(final Context context) {
			...
			mForceDividerToBeVisible = getResources_getBoolean(context, R.bool.abs__config_overflowMenuButtonForceDividerToBeVisible);
		...
		@Override
		public void addOnAttachStateChangeListener(final View_OnAttachStateChangeListener listener) {
		...
		public boolean needsDividerBefore() {
			return mForceDividerToBeVisible;
		}
		...
		protected void onAttachedToWindow() {
			...
			for (final View_OnAttachStateChangeListener listener : mListeners) {
		...
		@Override
		public void removeOnAttachStateChangeListener(final View_OnAttachStateChangeListener listener) {
	...
	private class OverflowPopup extends MenuPopupHelper {
		public OverflowPopup(final Context context, final MenuBuilder menu, final View anchorView,
				final boolean overflowOnly) {
	...
	public boolean flagActionItems() {
		...
		for (int i = 0; i < itemsSize; i++) {
			final MenuItemImpl item = visibleItems.get(i);
		...
		for (int i = 0; i < itemsSize; i++) {
			...
			if (item.requiresActionButton()) {
				final View v = getItemView(item, mScrapActionButtonView, parent);
				...
				if (isAction && groupId != 0) {
					...
					for (int j = 0; j < i; j++) {
						final MenuItemImpl areYouMyGroupie = visibleItems.get(j);
	...
	@Override
	public View getItemView(final MenuItemImpl item, View convertView, final ViewGroup parent) {
	...
	public boolean hideOverflowMenu() {
		...
		final MenuPopupHelper popup = mOverflowPopup;
		if (popup != null) {
			popup.dismiss();
			return true;
		}
	...
	public void onRestoreInstanceState(final Parcelable state) {
		...
		if (saved.openSubMenuId > 0) {
			final MenuItem item = mMenu.findItem(saved.openSubMenuId);
			if (item != null) {
	...
	public boolean showOverflowMenu() {
		if (mReserveOverflow && !isOverflowMenuShowing() && mMenu != null && mMenuView != null &&
			...
			final OverflowPopup popup = new OverflowPopup(mContext, mMenu, mOverflowButton, true);
	...
	public void updateMenuView(final boolean cleared) {
		...
		if (hasOverflow) {
			...
			final ViewGroup parent = (ViewGroup) mOverflowButton.getParent();
			if (parent != mMenuView) {
				if (parent != null) {
